...
(define (solve puzzle)
  ...
  (define firsts (remove-duplicates (map (lambda (w) (substring w 0 1)) words)))
  ...
  (define (is-first-letter-of-word-zero? p h)
    (ormap (lambda (i) (zero? (hash-ref h i #f))) firsts))
  ...
  (define first-solution
    (for/first ([p (generate-permutations number-set (length individual-letters))]
                #:do [(define h (construct-answer p))]
                #:when (and 
                         (not (is-first-letter-of-word-zero? p h))
                         (solution-found? p)))
